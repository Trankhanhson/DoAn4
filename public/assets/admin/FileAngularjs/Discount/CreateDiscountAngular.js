...
discountApp.controller("discountController", function ($scope, $http) {
    ...
    $scope.deleteProductSelect = function (index) {
        $scope.productList.forEach(element => {
            if(element.ProId == $scope.listProConfirmed[index].ProId){
                element.Check = false;
            }
        });
        ...
        if ($scope.listProConfirmed.length > 0) {
            if ($scope.discountForm.$valid) {
                ...
                if (checkDate) {
                    ...
                    let stopted = document.getElementById("EndDate").value;
                    let formData = new FormData();
                    formData.append('Name',$scope.discountPro.Name);
                    formData.append('StartDate', started)
                    formData.append('EndDate',stopted)
                    formData.append('listDiscountDetail',JSON.stringify(listDiscountDetail))
                    $http.post("/admin/discount/create",formData,{
