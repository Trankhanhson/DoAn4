...
function updateNewCart() {
    if (localStorage.getItem("Cart") != null) {
        ...
        formData.append('variationCarts',JSON.stringify(variationCarts))
        $.ajax({
            async: true,
            type: 'POST',
            url: "/client/cart/getNewCart",
        ...
        if (listCartItem != null) {
            ...
            if (check) {
                var data = {
                    ...
                    ProColorId: CartItem.proColorId,
                    ProSizeId: CartItem.proSizeId,
                    newQuantity: listCartItem[indexItem].Quantity + 1
                }
                var formData = new FormData();
                formData.append('_token', csrfToken); // Thêm CSRF token vào formData
