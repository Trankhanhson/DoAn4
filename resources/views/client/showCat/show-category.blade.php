...
<script>
    ClientApp.controller("ShowCatController", ShowCatController)
    function ShowCatController($scope, $http) {
        $scope.totalCount = 0;
        $scope.pageSize = 8;
        $scope.pageNumber = 1;
        $scope.MinPrice = -1
        $scope.MaxPrice = -1
        $scope.ColorId = 0
        let CatId = $("main").attr("data-id")
        $scope.ListProduct = []

        $scope.getData = function () {
            $http.get("/client/getCatData", {
                params: { CatId: CatId, colorId: $scope.ColorId, minPrice: $scope.MinPrice, maxPrice: $scope.MaxPrice, pageNumber: $scope.pageNumber, pageSize: $scope.pageSize }
            }).then(function (res) {
...
<main data-id="{{ $category->CatID }}">
    ...
    <div class="container-main">
        <ul class="breadcrumb mt-2 mb-2">
            ...
            @if ($category->type == "Nữ")
                <li class="breadcrumb-item"><a href="{{ route('client.women') }}">{{ $category->type }}</a></li>
            @elseif ($category->type == "Nam")
                <li class="breadcrumb-item"><a href="{{ route('client.man') }}">{{ $category->type }}</a></li>
            @elseif ($category->type == "Bé trai")
                <li class="breadcrumb-item"><a href="{{ route('client.baby_boy') }}">{{ $category->type }}</a></li>
            @elseif ($category->type == "Bé gái")
                <li class="breadcrumb-item"><a href="{{ route('client.baby_girl') }}">{{ $category->type }}</a></li>
            @endif
        ...
        <div class="row">
            <div class="col-md-2 col-md-block col-none category-left">
                ...
                @include('client.showCat.sidebar_showcat', ['Categories' => $Categories,'catId'=>$catId])
            </div>
            <div class="col-md-10 col-12" ng-controller="ShowCatController">
